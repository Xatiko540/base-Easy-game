...
- Все 17 уровней независимы: любой открытый уровень можно активировать в любом
  порядке, независимо от уже купленных верхних или нижних уровней.
- Повторно купить уже активированный уровень нельзя.
- Пропущенный уровень не даёт матричную позицию, билет, рейтинг или
  реферальную ёмкость. Их можно получить только если успеть активировать этот
  уровень до закрытия его окна входа.
- Freeze действует внутри конкретного раунда и не блокирует покупку других
  уровней.
...
- При достижении лимита новых прямых партнеров пригласитель должен
  активировать ещё одну более высокую ступень, чтобы получить еще четыре места.
...
Каждый тип и общий вес имеют caps. Если пригласитель еще не активен в этом
раунде, его реферальный вес сохраняется как pending и применяется при его
активации в том же раунде. Каждые принятые 100 единиц реферального веса дают
игроку дополнительный ticket и новую позицию в бинарной матрице. Остатки +50 и
+25 переносятся до следующего начисления. Поэтому приглашения дают больше
попыток занять одну из заранее зафиксированных winning cells. Если победителей
несколько, их итоговый вес определяет долю prize pool.
...
```text
1. Заполнить публичный season spec без приватных ключей.
2. SEASON_SPEC_PATH=... SEASON_MANIFEST_PATH=... npm run season:generate -- --network baseSepolia
3. CLI создает 17 Merkle roots, вычисляет freezeLimit и подписывает каждый config
   отдельным EIP-712 schedule signer из локального ignored environment.
4. SEASON_MANIFEST_PATH=... npm run season:commit -- --network baseSepolia
5. Дождаться подтверждения SeasonCommitted в Base.
6. Вызвать admin callable publishSeasonManifest с тем же неизмененным JSON.
7. Flutter принимает карточки только при совпадении Firestore и on-chain hashes.
```

Сгенерированный manifest содержит еще не раскрытые winning cells, создается с
правами `0600` и игнорируется Git. CLI не записывает и не печатает приватный
ключ schedule signer.
...
- активен;
- активен и заморожен;
- доступен любой открытый и ещё не купленный уровень;
